...
# Отключение проверки user-agent для сессии
...
# Логирование и сохранение сессии перед каждым запросом
@app.before_request
def log_and_preserve_session():
    user_agent = request.headers.get('User-Agent')
    app.logger.debug(f"User-Agent: {user_agent}, Session before: {session.get('user')}, {session.get('admin')}, {session.get('theme')}")
    
    user = session.get('user')
    admin = session.get('admin')
    theme = session.get('theme')
    
    if not session.get('user') and user:
        app.logger.debug("Сессия сброшена, восстанавливаем")
        session['user'] = user
        if admin:
            session['admin'] = admin
        if theme:
...
def load_groups():
    if os.path.exists('groups.json'):
        with open('groups.json', 'r', encoding='utf-8') as f:
            return json.load(f)
    return [
        {'name': 'Админ', 'color': '#ff0000', 'permissions': {'add_news': True, 'delete_news': True, 'edit_news': True, 'add_admins': True, 'manage_groups': True, 'manage_users': True, 'is_admin': True}},
        {'name': 'Premium', 'color': '#ffd700', 'permissions': {'add_news': False, 'delete_news': False, 'edit_news': False, 'add_admins': False, 'manage_groups': False, 'manage_users': False, 'is_admin': False}},
        {'name': 'Пользователь', 'color': '#ffffff', 'permissions': {'add_news': False, 'delete_news': False, 'edit_news': False, 'add_admins': False, 'manage_groups': False, 'manage_users': False, 'is_admin': False}}
    ]
...
def index():
    ...
    pinned_news = [n for n in news_list if n['slug'] in settings.get('pinned_slugs', [])]
    theme = session.get('theme', 'dark')
    categories = load_categories()
    icons = load_icons()
    return render_template('index.html', news_list=news_list, pinned_news=pinned_news, settings=settings, linkify=linkify, client_ip=request.remote_addr, theme=theme, categories=categories, icons=icons)
...
def news_detail(slug):
    ...
    return render_template('news_detail.html', news=news, comments=news_comments, linkify=linkify, client_ip=request.remote_addr, theme=theme, categories=categories, icons=icons, downloads=downloads)
...
def register():
    ...
    if request.method == 'POST':
        ...
        users.append({'login': login, 'password': password, 'group': 'Пользователь'})
        save_users(users)
        session['user'] = login
        session['admin'] = False
...
def login():
    if request.method == 'POST':
        ...
        if user:
            session['user'] = login
            groups = load_groups()
            user_group = user.get('group', 'Пользователь')
            group = next((g for g in groups if g['name'] == user_group), None)
            print(f"Поиск группы для пользователя: {user_group}")
            print(f"Найден пользователь: {user}")
            print(f"Найдена группа: {group}")
            
            if group:
                session['admin'] = group['permissions'].get('is_admin', False)
                session['premium'] = group['name'] == 'Premium'
            else:
                session['admin'] = False
                session['premium'] = False
...
def manage_users():
    ...
    if request.method == 'POST':
        ...
        if user and new_group in [g['name'] for g in groups]:
            user['group'] = new_group
            save_users(users)
            if session.get('user') == username:
                group = next((g for g in groups if g['name'] == new_group), None)
                if group:
                    session['admin'] = group['permissions'].get('is_admin', False)
                    session['premium'] = group['name'] == 'Premium'
    ...
    categories = load_categories()
    icons = load_icons()
    return render_template('admin.html', categories=categories, theme=theme, settings=load_settings(), icons=icons, show_manage_users=True, users=users, groups=groups)
...
def add_news():
    ...
    for screenshot in screenshots[:5]:
        if screenshot and allowed_file(screenshot.filename):
            ...
            screenshot_paths.append(f"uploads/{filename}")
...
def edit_news(slug):
    ...
    categories = load_categories()
    if request.method == 'POST':
        # Обработка удаления изображений и скриншотов
        if 'delete_images' in request.form:
            image_to_delete = request.form['delete_images']
            print(f"Попытка удаления файла: {image_to_delete}")
            # Удаление из списка изображений
            if image_to_delete in news.get('images', []):
                news['images'].remove(image_to_delete)
                full_path = os.path.join(app.config['UPLOAD_FOLDER'], image_to_delete.replace('uploads/', ''))
                print(f"Удаление изображения с пути: {full_path}")
                ...
                save_news(news_list)
                return redirect(url_for('edit_news', slug=slug))
            # Удаление из списка скриншотов
            if image_to_delete in news.get('screenshots', []):
                news['screenshots'].remove(image_to_delete)
                full_path = os.path.join(app.config['UPLOAD_FOLDER'], image_to_delete.replace('uploads/', ''))
                print(f"Удаление скриншота с пути: {full_path}")
                if os.path.exists(full_path):
                    try:
                        os.remove(full_path)
                    except Exception as e:
        ...
        if 'delete_attachments' in request.form:
            attachment_to_delete = request.form['delete_attachments']
            print(f"Попытка удаления вложения: {attachment_to_delete}")
            attachment_to_remove = next((a for a in news.get('attachments', []) if a['path'] == attachment_to_delete), None)
            if attachment_to_remove:
                news['attachments'].remove(attachment_to_remove)
                full_path = os.path.join(app.config['UPLOAD_FOLDER'], attachment_to_delete.replace('uploads/', ''))
                print(f"Удаление вложения с пути: {full_path}")
                if os.path.exists(full_path):
                    try:
                        os.remove(full_path)
                    except Exception as e:
                        print(f"Ошибка при удалении вложения: {e}")
                save_news(news_list)
                return redirect(url_for('edit_news', slug=slug))
        ...
        image_paths = news.get('images', []).copy()
        screenshot_paths = news.get('screenshots', []).copy()
        attachment_paths = news.get('attachments', []).copy()
        ...
        save_news(news_list)
        return redirect(url_for('edit_news', slug=news['slug']))
...
def download_file(filename):
    downloads = load_downloads()
    file_key = os.path.basename(filename)  # Извлекаем только имя файла
    downloads[file_key] = downloads.get(file_key, 0) + 1
    save_downloads(downloads)
    return send_from_directory(app.config['UPLOAD_FOLDER'], file_key, as_attachment=True)
...
def edit_comment(comment_id):
    ...
    if request.method == 'POST':
        ...
        if content:
            ...
            comment['edit_datetime'] = datetime.now().strftime('%d.%m.%Y %H:%M')
            comment['edited'] = True
            save_comments(comments)
...
@app.route('/admin/pin/<slug>')
def pin_news(slug):
    if 'admin' not in session:
        return redirect(url_for('admin'))
    settings = load_settings()
    settings.setdefault('pinned_slugs', []).append(slug)
    save_settings(settings)
    return redirect(url_for('index'))

@app.route('/admin/toggle_pin/<slug>')
def toggle_pin_news(slug):
    if 'admin' not in session:
        return redirect(url_for('admin'))
    settings = load_settings()
    settings.setdefault('pinned_slugs', [])
    if slug in settings['pinned_slugs']:
        settings['pinned_slugs'].remove(slug)
    else:
        settings['pinned_slugs'].append(slug)
    save_settings(settings)
    return redirect(url_for('index'))    
